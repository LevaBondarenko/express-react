...
if (browser.params.mobile.run) {
  sh.getCities().forEach(function(city){
    describe('Заголовок МВС', function() {
      ...
      it('Переключатель', function() {
        ...
        expect(switcher.getButtonsCount()).toBe(3,
          'В переключателе типов операций должно быть 3 кнопки');
        expect(isButtonActive(0)).toBe(true,
          'В переключателе должна быть активной перрая кнопка');
      ...
      describe('Выбор города', function() {
        ...
        it('Открытие диалога', function() {
          ...
          verifyDialog(true);
          header.cityList.toggle().then(function() {
            verifyDialog(false);
            header.cityList.toggle().then(function() {
              verifyDialog(true);
              header.closeDialog().then(function() {
                verifyDialog(false);
                header.cityList.toggle();
              });
            });
