...
async function generate() {
  ...
  const componentType = process.argv[4] === 'stateless' ?
    'stateless' : 'component';


  if (componentPath.length === 0) {
    console.error(
      'Error: Specify component name, for example: npm run generate todo'
    );
    return;
  }
  ...
  const variablePath = path.relative(
    componentPath, 'src/components/variables.scss'
  );
  ...
  fs.exists(componentPath, (isDirExist) => {
    ...
    mkdirp(componentPath, {}, () => {
      const templatePath = path.join('tools', 'templates', componentType);
